...
2) POST /api/generate — scripts + captions + 15–25 hashtags + affiliate links
3) Export JSON/CSV

Rate limiting + logging included.

Local: npm ci && npm run dev → /api/health -> {"ok":true}
`);
  }
}
...
function joinUrl(base, path, tag) {
  if (!base) return "";
