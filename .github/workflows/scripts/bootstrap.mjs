...
// Robust, idempotent bootstrap for the content generator MVP
...
async function ensurePkg() {
  if (!exists("package.json")) throw new Error("package.json missing. Scaffold step didn’t run.");
  const pkg = readJSON("package.json");

  pkg.name = pkg.name || "contentgen";
...
Includes basic rate limiting and logging.
...
export const platformMarker = (p: Platform) =>
...
export async function fetchTrends(region="US", limit=10): Promise<Trend[]> {
  ...
  return (j.items||[]).map((it:any,i:number)=>({
    keyword: it.snippet?.title || \`Video \${i+1}\`,
    score: 100 - i,
    source: "youtube"
  }));
}
...
export function rateLimit(key: string, limit=30, windowMs=60_000) {
  ...
  e.hits++;
  return { allowed: true, remaining: limit - e.hits, resetAt: e.resetAt };
}
...
export async function GET(req: NextRequest) {
  ...
  try {
    const items = await fetchTrends(region, limit);
    logReq("/api/trends", ip, 200, Date.now()-t0);
    return NextResponse.json({ region, limit, items });
  } catch (e:any) {
    logReq("/api/trends", ip, 500, Date.now()-t0);
...
export async function POST(req: NextRequest) {
  ...
  if (exportAs === "csv") {
    return new Response(toCSV(items), { headers: { "content-type": "text/csv" } });
  }
...
export default function Home() {
  ...
  async function run() {
    ...
    const r = await fetch("/api/generate", {
      ...
      body: JSON.stringify({ topic, tone, platforms: ["tiktok","reels","shorts"] })
    });
...
async function writeTests() {
  writeTxt("tests/prompts.test.ts", `
...
`.trimStart());
  ...
  writeTxt("tests/affiliate.test.ts", `
import { injectAffiliateLinks } from "../lib/affiliate";
import { describe, it, expect } from "vitest";
describe("affiliate", () => {
  it("finds links", () => {
